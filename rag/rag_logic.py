...
class RAGChatbot:
    ...
    def process_uploaded_file(self, file_obj) -> str:
        ...
        base_filename = os.path.basename(getattr(file_obj, 'orig_name', file_obj.name))

        if base_filename in self.processed_files:
            logger.warning(f"File '{base_filename}' has already been processed. Skipping reprocessing.")
            return f"File '{base_filename}' has already been processed. Please upload a different file or select this one from the drop down menu."

        logger.info(f"Starting processing for uploaded file: {base_filename}")

        try:
            filename, collection_id, embeddings_data = self.doc_processor.process_document(file_obj)

            if not filename or not collection_id:
                logger.error(f"Document processing failed for {base_filename}. Received: filename={filename}, collection_id={collection_id}, has_embeddings_data={bool(embeddings_data)}")
            ...
            logger.error(f"Error processing file {base_filename}: {e}", exc_info=True)
            self.processed_data = []
            self.processed_file_name = None
            return f"Error processing file '{base_filename}': {str(e)}"
